...
export default function Perfil({ navigation }) {
  ...
  async function loadRTData() {
    const suscriber = firestore().collection('InformacionPersonal').onSnapshot(querySnapshot => {
      const informacion = []
      querySnapshot.forEach(documentSanpshot => {
        informacion.push({
          ...documentSanpshot.data(),
          key: documentSanpshot.id
        })
      })
      setRTData(informacion)
    })
    return () => suscriber()
  }
  ...
  function renderRTItem({ item }) {
    return (
      ...
      >
        ...
        <Text>{item.apellidoPersona}</Text>
        <Text>{item.numeroTelefono}</Text>
      </TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.btnIcono}>
        ...
        <Image style={{ width: 130, height: 70, alignSelf: 'center' }}
